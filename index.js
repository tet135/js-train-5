...
function modifyArray(array) {
  delete array[2];
  return console.log(array.toString());  // Видаляємо третій елемент масиву
...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (const nextElement of [firstElement, ...restElements]) {
    if (nextElement < min) {
      min = nextElement;
    }
    if (nextElement > max) {
...
function concatenateStrings(array, subString) {
  let a = [array, subString].toString();
  return console.log(a.includes(subString));
...
function getEvenNumbers(arr) {
  ...
  for (const position in arr) {
      if (arr[position] % 2 === 0) {
        newArray[newArray.length] = arr[position];
      }
    }
  return newArray;
...
function replaceNumbers(arr) {
  ...
  for (const number of arr) {
    if (number > 10) {
      greaterNumbers[greaterNumbers.length] = "Greater than 10";
    } else {
